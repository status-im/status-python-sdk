...
- [`Community`](./community.md#communityaccount-community_idnone-urlnone) - manages membership (members, join requests, bans), the community's channels and reports its [minted tokens](./community.md#get_collectibles).
...
**This is destructive.** Everything inside the `data_folder` given when initializing[`class Community`](./community.md#communityaccount-community_idnone-urlnone) constructor is **deleted** and replaced with the contents of `folder`. Point `folder` at a copy of the account data, never at the only one you have.
